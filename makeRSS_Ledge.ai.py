...
async def main():
    ...
    for getURL in urls:
        ...
        # RSSフィードの初期化
        feed = Rss201rev2Feed(
            title=f"Ledge.ai {getURL.split('/')[-2]} カテゴリ",
            link=getURL,
            description=f"Ledge.aiの{getURL.split('/')[-2]}カテゴリの最新記事",
            language="ja",
            pretty=True
        )
        ...
        # RSSフィードをファイルに書き出し
        with open(f'feed_{getURL.split("/")[-2]}.xml', 'w') as f:
            feed.write(f, 'utf-8')

        print(f"{getURL.split('/')[-2]} カテゴリのRSSフィードが生成されました🎉")
